...
class IXPE_fits(object):
    def __init__(self, filename, du_id, NN=False, plot_modf=False, save_modf_plot=False, bg_prob=False):
        ...
        self.du_id = du_id
        self.set_irf = load_irf_set(du_id =self.du_id)
        if bg_prob:
            if self.NN:
    ...
    def make_orbital_LC_v2(self, nbins, Emin, Emax, Porb, TASC, weights=2):
        energy_cut= (Emin, Emax)
        binwidth=1/nbins
        ...
        self.total_image.phase = Convert_Phase_normalized(self.total_image.time, TASC, Porb)
        ...
        #exPer
        path = '/Users/asullivan/Stanford/J1723/IXPE/03006799/data_2025-04-12'
        hk_directory = f"{path}/hk"
        hks = [None, None, None]
        ...
        #source='03006799'
        #obs_id=1
        source = leakagelib_v3.source.Source.no_image(True) 
        for tbin in range(0, nbins):
            ...
            energy_mask=(Emin <= self.total_image.E) & (self.total_image.E < Emax)
            mask_phase = (self.phases_orb_LC[tbin]<=self.total_image.phase[energy_mask])& (self.phases_orb_LC[tbin]+binwidth>self.total_image.phase[energy_mask]) 
            exposure= np.sum(self.exposure_clean[phases_exposure])
            self.mask_phase_resolved.append(mask_phase)
            #print('exposure time: {}'.format(exposure ))
            self.exposure_time[tbin]=self.timebin*exposure 
            # True if you would like to use neural net-reconstructed data in the future, and False if you would like to use Mom-reconstructed data.
           # detector = leakagelib_v3.IXPEData(source, (self.filename, hks[self.du_id-1]), energy_cut,  weight_image=weight_image)
            #datas = leakagelib_v3.ixpe_data.IXPEData.load_all_detectors_with_path(source, path, weight_image=True)
            #datas[0][self.du_id-1].cut(mask_phase)
            #detector
            #for data in datas[0]:
            #    data.cut_region(f"/Users/asullivan/Stanford/IXPE/leakagelib_v3/examples/psf-region.reg")
            #    data.centroid_center()
            #detector.cut_region(f"/Users/asullivan/Stanford/IXPE/leakagelib_v3/examples/psf-region.reg")
            #detector.centroid_center()
            #result = leakagelib_v3.ps_fit.fit_point_source(datas[0], fixed_position=(0, 0), full_hessian=False)
            #result2 = leakagelib_v3.ps_fit.fit_point_source([detector], fixed_position=(0, 0), full_hessian=False)
            #print('phase: {}'.format(self.phases_orb_LC[tbin]+binwidth/2.))
            #print(result2)
...
class standard_pipeline(object):
    def __init__(self, filename,  du_id, srcd1_x, srcd1_y, srcd1_r, bkgd1_x, bkgd1_y, bkgd1_r, Pb, Ephemeris_IXPE, nbins = 5, timebin=1000, Emin=2, Emax=8, weights=1, use_weights=True, excise_bkg_flares=False, Emin_flare=2, Emax_flare=8, backgroundthresh=5, annulus=False, rin=0, compute_spectrum=False, specbins=10, bkg_sub=0, bg_prob=False, bg_prob_thresh=0.75):
        ...
        self.detector.make_orbital_LC_v2(nbins, Emin, Emax, Pb, Ephemeris_IXPE, weights=weights)
        if compute_spectrum:
...
class run_pipeline(object):
    def __init__(self, direct_list, srcd1_x_list, srcd1_y_list, srcd1_r_list, bkgd1_x_list, bkgd1_y_list, bkgd1_r_list, Pb_list, Ephemeris_IXPE_list, nbins = 5, timebin=1000, Emin=2, Emax=8, bkg_sub=0, annulus=False, bkgd1_rin=0, weights=1, use_weights=True, compute_spectrum=False, specbins=10, bg_prob=False, bg_prob_thresh=0.5):
        ...
        for k, direct in enumerate(direct_list):
            ...
            self.run_IXPE_leakage_pipeline(detector1.detector.phases_orb_LC, detector1.detector.mask_phase_resolved, detector2.detector.mask_phase_resolved, detector3.detector.mask_phase_resolved)
    ...
    def run_IXPE_leakage_pipeline(self, phases, det1_phase_mask, det2_phase_mask, det3_phase_mask, path = '/Users/asullivan/Stanford/J1723/IXPE/03006799/data_2025-04-12'):
        source = leakagelib_v3.source.Source.no_image(True)
        
        for n, phase in enumerate(phases):
            Data = leakagelib_v3.ixpe_data.IXPEData.load_all_detectors_with_path(source, path, weight_image=True)
  
            for i, data in enumerate(Data[0]):
                
                if i ==0:
                    mask_phase=det1_phase_mask[n]
                    data.cut(mask_phase)
                elif i==1:
                    mask_phase=det2_phase_mask[n]
                    data.cut(mask_phase)
                elif i==2:
                    mask_phase=det3_phase_mask[n]
                print(i)
            ...
            result = leakagelib_v3.ps_fit.fit_point_source(Data[0], fixed_position=(0, 0), full_hessian=True, mcmc=True)
