...
class IXPE_fits(object):
    ...
    def make_orbital_LC_leakege(self, nbins, Emin, Emax, Porb, TASC, weights=1, use_weights=True, bkg_sub=0):
        E_mask=np.where((self.src_photons.E>Emin) & ( self.src_photons.E<Emax))
        self.src_photons.phase = Convert_Phase_normalized(self.src_photons.time, TASC, Porb)
        
        binwidth=1./np.float(nbins)
        ...
        source = leakagelib-v5.source.Source.no_image(True)
        IXPE_data=leakagelib-v5.ixpe_data.IXPEData(source, (self.filename, self.hkname), energy_cut=(Emin, Emax))
        IXPE_data.cut(self.src_reg)
        ...
        for tbin in range(0, nbins):
            ...
            time_mask = np.where((tbin*binwidth<self.src_photons.phase[E_mask]) & ((tbin+1)*binwidth>self.src_photons.phase[E_mask]))
            
            IXPE_data_copy.cut(time_mask)
            settings = leakagelib.ps_fit.FitSettings([IXPE_data_copy])
...
class standard_pipeline(object):
    def __init__(self, filename, hkfilename,  du_id, srcd1_x, srcd1_y, srcd1_r, bkgd1_x, bkgd1_y, bkgd1_r, Pb, Ephemeris_IXPE, nbins = 5, timebin=1000, Emin=2, Emax=8, weights=1, use_weights=True, excise_bkg_flares=False, Emin_flare=2, Emax_flare=8, backgroundthresh=5, annulus=False, rin=0, compute_spectrum=False, specbins=10, bkg_sub=0, bg_prob=False, bg_prob_thresh=0.75):
        ...
        self.detector.make_orbital_LC_leakege(nbins, Emin, Emax, Porb, TASC)(nbins, Emin, Emax, Pb, Ephemeris_IXPE, weights=weights, use_weights=use_weights, bkg_sub=bkg_sub)
        if compute_spectrum:
