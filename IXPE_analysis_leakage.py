...
class IXPE_fits(object):
    ...
    def make_LC_v2(self, timebin, Emin, Emax, weights=1):
        '''weights can be one of 0 (meaning no weights), 1 (meaning moments), and 2 (meaning neural net weights)'''
        ...
        times_use=self.total_image.time[Echoose]
        print
        #print(max_time-min_time)%timebin
        numbins=int((max_time-min_time)/timebin+1)
        ...
        self.time_index=np.empty(len(times_use))
        print("time index_length",format(len(self.time_index)))
        self.bin_list=np.array(range(0, numbins))
        for tbins in range(0, numbins):
            time_mask = np.where((self.t_LC[tbins]<=times_use) & (self.t_LC[tbins+1]>times_use), True, False)
            
            self.total_counts_LC[tbins]=len(times_use[time_mask])
    ...
    def make_orbital_LC_leakege(self, nbins, Emin, Emax, Porb, TASC, weights=1, use_weights=True, bkg_sub=0):
        ...
        source = leakagelib_v8.source.Source.no_image(True)
        IXPE_data=leakagelib_v8.ixpe_data.IXPEData(source, (self.filename, self.hkname), energy_cut=(Emin, Emax))
        ...
        for tbin in range(0, nbins):
            ...
            settings = leakagelib_v8.ps_fit.FitSettings([IXPE_data_copy])
            ...
            fitter = leakagelib_v8.ps_fit.Fitter([IXPE_data_copy], settings)
            result = fitter.fit()
...
class run_pipeline_leakage_v2(object):
    def __init__(self, direct_list, hk_direct_list, srcd1_x_list, srcd1_y_list, srcd1_r_list, bkgd1_x_list, bkgd1_y_list, bkgd1_r_list, Pb_list, Ephemeris_IXPE_list, det_use = [1, 2, 3], nbins = 5, timebin=1000, Emin=2, Emax=8, bkg_sub=0, annulus=False, bkgd1_rin=0, weights=1, use_weights=True,  compute_spectrum=False, specbins=10, bg_thresh =5, bg_prob=False, bg_prob_thresh=0.5, combine_all=True, combine_detectors_leakage=True, model_fit = False, model_i=None, model_q=None, model_u=None, model_prior=[0.6, 0.03], model_unpolarized=False, fit_const=False, fit_const_qu=False, q_model_const=None, u_model_const=None,const_qu_prior=[0.0, 0.0], bg_weights=True, bg_cut=False, fit_bg_qu=False, fit_bg_det=False, fit_bg_det_sep=True, src_remove=20, make_lc=True, flare_cut=True, show_hist=False, cut_amount=35, fit_prtl_bg=True, fit_prtl_qu=True, do_fit=True, hist_xlim=[0, 100], hist_ylim=[0, 100], hist_bins=100, refit_bkg=True):
        ...
        if combine_detectors_leakage:        
            if flare_cut:
                self.flare_cut(list_dataset_unpacked, timebin, Pb_list[0], Ephemeris_IXPE_list[0], Emin=Emin_list[0], Emax =Emax_list[0],  weights=weights_list[0], use_weights=True, bkg_sub=use_weights_list[0], show_hist=show_hist, cut_amount=cut_amount, xlim=hist_xlim, ylim=hist_ylim, hist_bins=hist_bins)
            
            if model_fit:
                if fit_bg_det and fit_bg_det_sep:
                    self.IXPE_bg_fit(list_dataset_unpacked, src_remove, nbins, Pb_list[0], Ephemeris_IXPE_list[0], Emin=Emin_list[0], Emax=Emax_list[0], weights=weights_list[0],  bg_weights=bg_weights, bg_cut=bg_cut)
                self.IXPE_lc_fit(list_dataset_unpacked, model_i, model_q, model_u, nbins, Pb_list[0], Ephemeris_IXPE_list[0], Emin=Emin_list[0], Emax=Emax_list[0], weights=weights_list[0], use_weights=use_weights_list[0], model_prior=model_prior, model_unpolarized=model_unpolarized, fit_const=fit_const, fit_const_qu=fit_const_qu, const_qu_prior=const_qu_prior, q_model_const=q_model_const, u_model_const=u_model_const, bg_weights=bg_weights, bg_cut=bg_cut,  fit_bg_qu=fit_bg_qu, fit_bg_det=fit_bg_det, fit_bg_det_sep=fit_bg_det_sep, flare_cut=flare_cut, fit_prtl_bg=fit_prtl_bg, fit_prtl_qu=fit_prtl_qu, do_fit=do_fit)
            
            if make_lc:
                self.make_orb_LC(list_dataset_unpacked, nbins, Pb_list[0], Ephemeris_IXPE_list[0], Emin=Emin_list[0], Emax=Emax_list[0], weights=weights_list[0], use_weights=use_weights_list[0],  fit_bg_qu=fit_bg_qu, fit_bg_det=fit_bg_det, fit_bg_det_sep=fit_bg_det_sep, model_fit=model_fit, flare_cut=flare_cut, fit_prtl_bg=fit_prtl_bg, fit_prtl_qu=fit_prtl_qu, refit_bkg=refit_bkg)
            
                self.phases=[self.phases_orb_LC]*len(direct_list)
                if combine_detectors_leakage:
                    self.I_combined_total = self.flux_orb_LC_src*self.N_perbin/(1+self.flux_orb_LC_src)
                    self.Ierr_combined_total = self.error_orb_LC_src*self.N_perbin/(1+self.flux_orb_LC_src)
                ...
                self.I_src = [self.I_combined_total]*len(direct_list)
                self.Ie_src = [self.Ierr_combined_total]*len(direct_list)
                self.Q_src = [self.Q_combined_total]*len(direct_list)
                self.Qe_src = [self.Qerr_combined_total]*len(direct_list)
                self.U_src = [self.U_combined_total]*len(direct_list)
                self.Ue_src = [self.Uerr_combined_total]*len(direct_list)
    ...
    def flare_cut(self, list_dataset_unpacked, timebin, Porb, TASC, Emin=2, Emax =8,  weights=1, use_weights=True, bkg_sub=0, fit_bg_qu=False, fit_bg_det=False, fit_bg_det_sep=True, model_fit=False, show_hist=True, cut_amount=35, xlim=[0, 100], ylim=[0, 100], hist_bins=100):
            ...
            if weights == 1:
                ...
                NN=True
            source = leakagelib_v8.source.Source.no_image(NN)
            ...
            for dataset in list_dataset_unpacked:
               ...
               dataset.make_LC_v2(timebin, Emin, Emax)
               ...
               hist_cts=dataset.total_counts_LC
               
               if show_hist:
                   plt.hist(hist_cts, bins =hist_bins)
                   plt.xlim(xlim[0], xlim[1])
                   plt.ylim(ylim[0], ylim[1])
                   
               keep_counts = np.where(dataset.total_counts_LC>cut_amount, False, True)
    ...
    def make_orb_LC(self, list_dataset_unpacked, nbins, Porb, TASC, Emin=2, Emax =8,  weights=1, use_weights=True, bkg_sub=0, fit_bg_qu=False, fit_bg_det=False, fit_bg_det_sep=True, model_fit=False, flare_cut=True, fit_prtl_bg=True, fit_prtl_qu=True, refit_bkg=True):
        binwidth=1./(nbins)
        N_perbin=np.empty(nbins)
        self.phases_orb_LC=np.linspace(0.0, 1.0-binwidth, nbins)
        self.flux_orb_LC_src = np.empty(nbins)
        ...
        for tbin in range(0, nbins):
            ...
            for dataset in list_dataset_unpacked:
                ...
                IXPE_data=leakagelib_v8.ixpe_data.IXPEData(source, (dataset.filename, dataset.hkname), energy_cut=(Emin, Emax))
                if flare_cut:
            ...
            settings = leakagelib_v8.ps_fit.FitSettings(IXPE_full_dataset)
            settings.add_point_source() # Point source component
            settings.add_background() # Background component
            #settings.fix_qu("bkg", (0, 0)) # Set the background to be unpolarized
            if fit_bg_qu:
                if fit_bg_det:
                    ...
                    if model_fit or fit_bg_det_sep:
                        if not refit_bkg:
                        ...
                        #settings.fix_flux("bkg1", (self.flu, self.u_det1_bkg))
                        ...
                        if fit_prtl_bg:
                            if fit_prtl_qu:
                                if not refit_bkg:
                                    ...
                                    settings.fix_qu("prtl2", (self.q_det2_pbkg, self.u_det2_pbkg))
                                    settings.fix_qu("prtl3", (self.q_det3_pbkg, self.u_det3_pbkg))
                            else:   
                                settings.fix_qu("prtl1", (0., 0.))
                                settings.fix_qu("prtl2", (0., 0.))
                                settings.fix_qu("prtl3", (0., 0.))
                            if not refit_bkg:
                                settings.fix_flux("prtl1", self.f_det2_pbkg)
            ...
            fitter = leakagelib_v8.ps_fit.Fitter(IXPE_full_dataset, settings)
            result = fitter.fit()
            #phases_exposure=np.where((self.phases_orb_LC[tbin]<self.phases_LC_tot)& (self.phases_orb_LC[tbin]+binwidth>self.phases_LC_tot))
    ...
    def IXPE_bg_fit(self, list_dataset_unpacked, src_remove, nbins, Porb, TASC, Emin=2, Emax =8,  weights=1, use_weights=True, bkg_sub=0, fit_sep_det=True, bg_weights=True, bg_weights_thresh=0.99, bg_cut=True, flare_cut=True):
        '''
        ...
        for dataset in list_dataset_unpacked:
            ...
            src_remove_mask = np.where(IXPE_data.evt_xs**2+ IXPE_data.evt_ys**2 < src_remove**2, 0, 1)
            IXPE_data.retain(src_remove_mask)
            #IXPE_data_copy = copy.deepcopy(IXPE_data)
            if not bg_weights:
        ...
        settings.add_background("bkg1", det=(1,)) # Background component from detector 1
        settings.add_background("bkg2", det=(2,)) # Background component from detector 1
        settings.add_background("bkg3", det=(3,)) # Background component from detector 1
    ...
    def IXPE_lc_fit(self, list_dataset_unpacked, lc_model, q_model, u_model, nbins, Porb, TASC, Emin=2, Emax =8,  weights=1, use_weights=True, bkg_sub=0, model_unpolarized=False, model_prior=[0.3, 0.3], fit_const=False, q_model_const=None, u_model_const=None, fit_const_qu=False, const_qu_prior=[0, 0], fit_bg_qu=False, fit_bg_det=False, fit_bg_det_sep=True, bg_weights=True, bg_weights_thresh=0.99, bg_cut=True, flare_cut=True, fit_prtl_bg=True, fit_prtl_qu=True, do_fit=True):
        ...
        if do_fit:
            ...
            if fit_const:
                ...
                if not fit_const_qu:
                    ...
                    if q_model_const is not None and u_model_const is not None:
                        ...
                        settings.set_sweep("const", qu_model_const)
            ...
            if model_unpolarized:
                ...
                settings.set_sweep("src", qu_model)
...
def plot_polarization(list_pipeline, list_labels, list_colors, shift=True, norm_flux = True, plot2orb=False, pd_ylim=[0, 0.4], desiredcapsize=3, save_fig=False, savename='polarization_plot', dpi=300, desiredfontsize=16, plot_bkg=False, plot_MDP=True, combine_all=False, no_show=False, combine_color='black', combine_color2='darkmagenta', list_pipeline2=None, list_labels2=None, list_colors2=None, combine_title='Total', combine_title2='Total2', model1 = None, model1color='blue', model_offset=None, model2= None, model2color='orange', i_ibs_model=None, i_const_model=None, q_ibs_model1=0., u_ibs_model1=0., q_const_model1=0, u_const_model1=0, q_ibs_model2=0., u_ibs_model2=0., q_const_model2=0, u_const_model2=0):
    ...
    if model1 is not None:
        ...
        if i_ibs_model is not None:
            q_ibs= q_calculate(model1[:, 2], model1[:, 3])*q_ibs_model1
            u_ibs= u_calculate(model1[:, 2], model1[:, 3])*u_ibs_model1
    ...
    if model2 is not None:
        ...
        if i_ibs_model is not None:
            q_ibs= q_calculate(model2[:, 2], model2[:, 3])*q_ibs_model2
            u_ibs= u_calculate(model2[:, 2], model2[:, 3])*u_ibs_model2
