...
class IXPE_fits(object):
    ...
    def make_orbital_LC_v2(self, nbins, Emin, Emax, Porb, TASC, weights=2):
        ...
        for tbin in range(0, nbins):
            ...
            mask_phase = np.where((self.mask_src) & (self.phases_orb_LC[tbin]<self.total_image.phase)& (self.phases_orb_LC[tbin]+binwidth>self.total_image.phase))
            ...
            detector = leakagelib_v3.IXPEData(source, (self.filename, hks[self.du_id]), energy_cut,  weight_image=weight_image)
            detector.cut(mask_phase)
            detector.cut_region(f"/Users/asullivan/Stanford/IXPE/leakagelib_v3/examples/psf-region.reg")
            detector.centroid_center()
            
            result = leakagelib_v3.ps_fit.fit_point_source([detector], fixed_position=(0, 0), full_hessian=True)
            print('phase: {}'.format(self.phases_orb_LC[tbin]))
            print(result)
